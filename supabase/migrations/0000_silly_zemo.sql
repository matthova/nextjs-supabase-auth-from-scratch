...
CREATE POLICY "owner can read and update" ON "counts_table" AS PERMISSIVE FOR ALL TO "authenticated" USING ("counts_table"."user_id" = (select auth.uid())) WITH CHECK ("counts_table"."user_id" = (select auth.uid()));